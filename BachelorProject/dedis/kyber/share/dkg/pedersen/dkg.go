...
func NewDistKeyGenerator(suite Suite, longterm kyber.Scalar, participants []kyber.Point, t int) (*DistKeyGenerator, error) {
	...
	ownSec := suite.Scalar().Pick(suite.RandomStream())  //THIS HAS TO BE 0
...
func (d *DistKeyGenerator) DistKeyShare() (*DistKeyShare, error) {
	...
	d.qualIter(func(i uint32, v *vss.Verifier) bool {
		...
		deal := v.Deal()  // SessionID || PriShare generated by d(dealer) || threshold || Commitments to verify share against
		s := deal.SecShare.V //share given by dealer j to verifier i, i.e fj(i).
		sh = sh.Add(sh, s)
